...
function applyTranslations(lang) {
    ...
    // Update any dynamic content that needs re-rendering
    updateBalanceDisplay();
    updateStats();
    // Re-render current log if open
...
async function performBackup() {
    ...
    try {
        ...
        const fileName = `${translate('backupFileNamePrefix')}_${timestamp}.json`;
...
function openTab(id, keepEdit = false) {
    ...
    if (editMode) {
        const indicatorMap = { inc: 'incEditIndicator', exp: 'expEditIndicator', rig: 'rigEditIndicator',
...
function _renderDetailContent(o, type) {
    ...
    if (o.صورة && type === 'exp') {
        html += `<div class="card" style="border-top-color:var(--s);">
            <h3 style="color:var(--s);margin-top:0;"><i class="fas fa-image" style="margin-left:5px;"></i> ${translate('invoiceImage')}</h3>
            <img src="${o.صورة}" alt="${translate('invoice')}" style="width:100%;border-radius:10px;margin-top:10px;box-shadow:var(--shadow-light);" />
            </div>`;
    }
    html += `<div style="display:flex;gap:10px;margin-top:20px;">
            <button class="secondary" onclick="editTransaction()" style="flex:1;"><i class="fas fa-edit" style="margin-left:5px;"></i> ${translate('edit')}</button>
            <button class="action" onclick="deleteTransaction()" style="background:var(--danger);flex:1;"><i class="fas fa-trash" style="margin-left:5px;"></i> ${translate('delete')}</button>
            </div>`;
    el.innerHTML = html;
}
    ...
    el.innerHTML = filtered.map(i => {
        ...
        if (isInc) {
            ...
            if (st.includes('كامل') || st === 'مدفوع بالكامل') { borderColor = 'var(--success)';
                statusBadge = `<span class="status-badge paid">${translate('statusPaid')}</span>`; } else if (st.includes('جزئياً') || st ===
                'مدفوع جزئياً') { borderColor = 'var(--warning)';
                statusBadge = `<span class="status-badge partial">${translate('statusPartiallyPaidShort')}</span>`; } else if (st === 'متأخر') {
                borderColor = '#e67e22';
...
function initDB() {
    return new Promise((resolve, reject) => {
        if (!window.indexedDB) { toastMsg(translate('indexedDBUnsupported'), "error"); return reject(new Error(
