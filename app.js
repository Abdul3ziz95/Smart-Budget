...
// ✔ ترجمة قيمة الحالة المخزنة بأي لغة → عرضها باللغة الحالية
...
if (!rAmount.value || !rType.value || !rDueDate.value) return toastMsg(translate('fillRequired'), "error");
 ...
 data.وقت_التنبيه = rNotifTiming ? (rNotifTiming.value || '168') : '168'; // الافتراضي: 7 أيام
...
if (!dType.value || !dDueDate.value) return toastMsg(translate('fillRequired'), "error");
 ...
 data.وقت_التنبيه = dNotifTiming ? (dNotifTiming.value || '168') : '168'; // الافتراضي: 7 أيام
 let paidAmount = 0;
...
const type = savedEdit.type;
 ...
 closeAllLayers();
 editMode = savedEdit; // ✔ الاستعادة الفورية (كان يُفقد فيصبح الحفظ معاملة جديدة)
 openTab(tabId, true);
 setTimeout(() => {
     if (type === 'inc') {
...
// البحث عن العنصر الأصلي في db
 ...
 `;
 // عرض جميع حقول العنصر الأصلي (✔✔✔ مترجمة العناوين والقيم)
 if (source) {
